...
if ($_GET['statusBtn']){
	...
	$update="UPDATE rent_payment SET Payment_status='Approved',payment_remarks='Payment Accepted' WHERE payment_no='$paymentID'";
			if (mysqli_query($conn,$update)){
				...
				$update="UPDATE inspection_report SET fine_status='Payment Approved' WHERE notice_id='$hnoticeID'";
			mysqli_query($conn,$update);
				...
				<input type="hidden" name="hnoticeID"value="'.$hnoticeID.'">
...
if(isset($_GET['rejectBtn'])){
	...
	$hnoticeID=$_GET['hnoticeID'];
// update inspection report table
	$update="UPDATE inspection_report SET fine_status='Rejected' WHERE notice_id='$hnoticeID'";
			mysqli_query($conn,$update);
	//update bookings
	$update="UPDATE rent_payment SET Payment_status='Rejected',payment_remarks='$comment' WHERE payment_no='$paymentID'";
			if (mysqli_query($conn,$update)){
				$sweet='success';
				$feedback='Payment Rejected';
			}
}
?>
...
<html lang="en">
  <head>
    ...
    <title>Fine Pending Approval </title>
...
echo"<script>
  ...
  <section id="container" class="">
    ...
    <input type="text" class="search form-control btn-warning" placeholder="Search Here?" style="color: black;background-color: white">
					...
					<div class="col-md-3">
						<div class=""> Fine Pending Approval</div></div>
...
<?php
	...
	$select="SELECT * FROM bookings AS b INNER JOIN rooms AS r ON b.room_id=r.room_id RIGHT JOIN apartment AS a ON r.apartment_id=a.apartment_id RIGHT JOIN rent_payment AS rp ON b.booking_id=rp.booking_id RIGHT JOIN tenants AS t ON b.tenant_id=t.tenant_id RIGHT JOIN vacate_notice AS v ON b.booking_id=v.booking_id WHERE rp.payment_status='Pending Approval' AND rp.type_of_payment='Fine Payment'";
