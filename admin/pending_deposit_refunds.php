...
if ($_POST['updateBtn']){
	 $bookingId=$_POST['hbookingID'];
	
	$update="UPDATE bookings SET deposit_status='Refunded' WHERE booking_id='$bookingId'";
...
<html lang="en">
  <head>
    ...
    <title>Pending Deposit Refund</title>
...
echo"<script>
  ...
  <section id="container" class="">
    ...
    <input type="text" class="search form-control btn-warning" placeholder="Search Here?" style="color: black;background-color: white">
					...
					<div class="col-md-3">
						<div class=""><h4> Deposit Refund</h4></div></div>
		       <div class="col-md-6">
...
<?php
	...
	$select="SELECT * FROM vacate_notice AS vn INNER JOIN rooms AS r ON vn.room_id=r.room_id RIGHT JOIN apartment AS a ON r.apartment_id=a.apartment_id RIGHT JOIN bookings as b ON vn.booking_id=b.booking_id RIGHT JOIN tenants AS t ON t.tenant_id=vn.tenant_id WHERE b.booking_status='Vacated'  AND b.deposit_status='Pending Refund'  OR b.deposit_status='Refunded'  ";
		...
		<td>'.$dueDate=date('d-F-Y',strtotime($row['rent_due_date']));
					...
					echo '</td>';								
					echo '<td>';
						 $refund= ( $row['rent']+$row['rent_deposit']* $mths)+$row['rent_balance'];
					if($refund>0){
					...
					echo	'	<form method="post">
						...
						<input type="submit" name="updateBtn" class="btn btn-sm btn-primary" value="Update">
						</form>';
						}
					echo '</td>
